  ...
  window.addEventListener("popstate", function(e) {
    ...
    if(localStorage.getItem("form")=="false"){
      // alert("我监听到了浏览器的返回按钮事件啦");//根据自己的需求实现自己的功能
	...
	function getAddressList(){
        ...
        crossDomain: true,
          ...
          var mobile;
            ...
            if(form=="false"){
              $($(".l_myaddress")[i]).not(".l_myaddress_edit").not(".l_delete").click(function(){
                if($(this).find(".l_myaddress_detail").attr("disabled")){
                  ...
                  $(".address_prime").addClass("hide");
                  $(this).find(".address_prime").removeClass("hide");
            ...
            $($(".l_delete")[i]).on("click",function(e){
              e.stopPropagation();
              if(item.isDefault=="1"){
            ...
            $("span.address_prime:first").removeClass("hide");
  ...
  function move(){
  	$(".l_myaddress_adder").on("swipeleft",function(){
  		$(this).stop().animate({marginLeft:'-20%'});
  		$(this).parent().parent().siblings().find(".l_myaddress_adder").animate({marginLeft:'0'});
  	});
